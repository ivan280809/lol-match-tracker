...
class PlayerDataServiceTest {
    ...
    @Mock
    private MatchServiceWebClient matchServiceWebClient;
    ...
    // datos de prueba
    private final List<PlayerJson> testPlayers = List.of(new PlayerJson("Test", "EUW"));

    @BeforeEach
    void setUp() {
        // inyectamos la lista inicial (vacía) para luego sobreescribirla en loadPlayers()
        playerDataService = new PlayerDataService(
                riotApiClient,
                matchServiceWebClient,
                objectMapper,
    ...
    void getSummonerData_ShouldReturnMatchData() throws Exception {
        ...
        when(riotApiClient.getMatchesByPuuid(anyString()))
                .thenReturn(Mono.just(matchIdsJson));
        when(riotApiClient.getMatchById(anyString()))
                .thenReturn(Mono.just(matchJson));
        when(matchServiceWebClient.putMatches(any()))
                .thenReturn(Mono.just("OK"));

        // Mocks de ObjectMapper
        AccountDTO acct = new AccountDTO();
        acct.setPuuid("puuid-123");
        doReturn(acct)
                .when(objectMapper)
                .readValue(eq(summonerResponse), eq(AccountDTO.class));
        ...
        doReturn(new MatchDto())
                .when(objectMapper)
                .readValue(eq(matchJson), eq(MatchDto.class));
    ...
    void refreshPlayerData_ShouldCallPutMatchesForEachPlayer() throws JsonProcessingException {
        // Inyectamos dos players en el servicio
        List<PlayerJson> two = List.of(
                new PlayerJson("A","T"),
                new PlayerJson("B","U")
        );
        ...
        // Mocks API
        when(riotApiClient.getSummonerByNameAndTagLine(any(), any()))
                .thenReturn(Mono.just("{}"));
        when(riotApiClient.getMatchesByPuuid(any()))
                .thenReturn(Mono.just("[]"));
        when(riotApiClient.getMatchById(any()))
                .thenReturn(Mono.just("{}"));

        // Mocks ObjectMapper para evitar el null-type
        doReturn(new AccountDTO())
                .when(objectMapper)
                .readValue(anyString(), eq(AccountDTO.class));
        doReturn(List.of("x"))
                .when(objectMapper)
                .readValue(anyString(), ArgumentMatchers.<TypeReference<List<String>>>any());
        doReturn(new MatchDto())
                .when(objectMapper)
                .readValue(anyString(), eq(MatchDto.class));
        ...
        // Debe haber llamado 2 veces (uno por cada player)
