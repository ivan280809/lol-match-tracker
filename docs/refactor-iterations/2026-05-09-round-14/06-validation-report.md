...
- P0: Ensure the deploy environment always runs Flyway before JPA validation. Addressed by explicit deploy environment variables; must be confirmed in the next GitHub Actions deploy run that reaches application startup.
- P0: Keep production DB migrations complete for existing rank/match tables. Current validation loads the local migration chain; production confirmation depends on the next deploy run.
- P0: Avoid Docker Hub pull failures blocking app deploys. Addressed by changing CI deploy to pull only the app image before restart.
...
- Production Flyway behavior must still be confirmed by the next remote deploy run that reaches the application health check.
