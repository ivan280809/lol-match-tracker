...
- P0: Ensure the deploy environment always runs Flyway before JPA validation. Addressed by explicit deploy environment variables; must be confirmed in the next GitHub Actions deploy run.
...
- Production Flyway behavior must still be confirmed by the next remote deploy run.
